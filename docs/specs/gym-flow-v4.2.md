...
### Completion state
- strength exercises show completed-set count in the active workout list
- prescribed-set completion gives a clear green completed state
- completing the final prescribed strength set gives a short completion confirmation and returns to the workout
- cardio completion also appears as completed in the workout list

### Strength set usability
- KG and REPS hit areas are larger and visually explicit
...
- one-arm / single-arm exercise names display `KG/ARM` and clarify that load is entered per working arm

### Quiet set timing foundation
- Forge records an `intent_started_at` timestamp once a set has both load and reps entered
...
- exercises categorised as Cardio use a timed-cardio surface instead of KG + REPS
- one-tap cardio start/stop with timestamp-derived elapsed time
...
3. Add Rowing to the active session only; verify it does not require KG/REPS.
4. Start/stop cardio and enter distance; verify duration/distance remain after navigating away/back.
5. Reorder two exercises because of equipment availability; verify the active order changes and survives navigation.
6. Log a normal strength exercise and verify completed-set count updates on Train.
7. Complete the final prescribed set; verify completion confirmation and return to the workout list.
8. Add an extra set, then delete it before completion.
9. Test a one-arm exercise and confirm KG/ARM wording is clear.
10. Enter load/reps quickly and confirm the first digit is not dropped.
11. Finish the session and verify the completion summary duration, exercises and sets are sensible.
12. Reopen Train and confirm Forge has advanced to the next rotation session without missed-day/streak language.
13. Confirm Progress still reads only real completed training data.
